...
class LuceneSearchPopulator implements SearchPopulator {
    ...
    LuceneSearchPopulator(Directory directory) {
        try {
    ...
    private Document makeDoc(Media media) {
        ...
        doc.add(new StringField(LuceneSearchProvider.F_ID, media.getId(), Store.YES));
        doc.add(new StringField(LuceneSearchProvider.F_SOURCE_ID, media.getSourceId(), Store.YES));
        doc.add(new StringField(LuceneSearchProvider.F_URI, media.getUri().toString(), Store.YES));
        doc.add(new LongField(LuceneSearchProvider.F_LAST_MODIFIED, media.getLastModifiedMs(), Store.YES));

        doc.add(new StringField(LuceneSearchProvider.F_ARTIST, blankNotNull(media.getArtist()), Store.YES));
        doc.add(new StringField(LuceneSearchProvider.F_ALBUM, blankNotNull(media.getAlbum()), Store.YES));
        doc.add(new StringField(LuceneSearchProvider.F_GENRE, blankNotNull(media.getGenre()), Store.YES));
        doc.add(new IntField(LuceneSearchProvider.F_YEAR, media.getYear(), Store.YES));
        doc.add(new StringField(LuceneSearchProvider.F_REQUESTED_BY, blankNotNull(media.getRequestedBy()), Store.YES));
        doc.add(new StringField(LuceneSearchProvider.F_TITLE, blankNotNull(media.getTitle()), Store.YES));
        ...
        addFacet(doc, LuceneSearchProvider.F_FACET_DECADE, floorYearToDecade(media.getYear()));

        doc.add(new TextField(LuceneSearchProvider.F_TEXT, makeSearchText(media)));
